...
def working_with_the_database(registered_channels=None):
    ...
    try:
        pickle.load(open(BotConfig.BotDataBasePath, "rb+"))
    # if a database fault is detected, data is reset, and the structure is restored
        ...
        pickle.dump(
            ...
            open(BotConfig.BotDataBasePath, "wb+"))
    # uploading updated data to the database if required
    if registered_channels != None:
        pickle.dump(registered_channels, open(BotConfig.BotDataBasePath, "rb+"))
    # returning the current stored database
    return pickle.load(open(BotConfig.BotDataBasePath, "rb+"))
...
client.run(str(open(BotConfig.BotTokenPath, "r+").readline()))
